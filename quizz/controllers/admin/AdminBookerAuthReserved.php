...
class AdminBookerAuthReservedController extends ModuleAdminControllerCore
{
    protected $_module = NULL;
	public $controller_type='admin';   
    protected $position_identifier = 'id_reserved'; //this filed is required if you use position for sorting
    ...
    public function __construct()
    {
        ...
        $this->_defaultOrderBy = 'id_reserved';
       ...
       // Shop::addTableAssociation($this->table, array('type' => 'shop'));
        ...
        $this->fields_list = array(
            'id_reserved' => array('title' => ('ID Reserved'), 'filter_key' => 'a!id_reserved', 'align' => 'center','remove_onclick' => true),       
            'id_booker' => array('title' => ('ID Booker'), 'filter_key' => 'a!id_booker', 'align' => 'center','remove_onclick' => true),           
            'date_reserved' => array('title' => ('Date'), 'filter_key' => 'a!date_reserved', 'align' => 'center','remove_onclick' => true),
            'hour_from' => array('title' => ('From'), 'filter_key' => 'a!hour_from', 'align' => 'center','remove_onclick' => true),
            'hour_to' => array('title' => ('To'), 'filter_key' => 'a!hour_to', 'align' => 'center','remove_onclick' => true),			
	...
	public function editableCombinationPriceMethod($value, $question)
    {
		...
		$this->context->smarty->assign('id', (int)$question['id_reserved']);
		return $this->context->smarty->fetch($this->getTemplatePath().'editable_field.tpl');
    }	
	...
	public function ajaxProcessManualPosition() {		
		...
		$sql="UPDATE `"._DB_PREFIX_."booker` set manual_position=".$valeur." where id_reserved=".$id;
	...
	/* public function postProcess()
	{
		//if ((Tools::isSubmit('manual_position_post')) == true) {
			print_r($_POST);
			$id_reserved = (int)Tools::getValue('id_reserved_ajax');
			$manual_position = (int)Tools::getValue('manual_position_ajax');
			$sql="UPDATE `"._DB_PREFIX_."booker` set manual_position=".$manual_position." where id_reserved=".$id_reserved;
			$manual_position = Db::getInstance()->query($sql); 
		//}		
		parent::postProcess();
		return false;
